...
{
	int numero_de_huesos : packoffset(c0) : OBJETO_BONES_NUMBER;
	Paleta4x4 paletas[50] : packoffset(c1) : OBJETO_BONES_PALETTE;
	matrix matriz_normal : packoffset(c201) : OBJETO_MATRIZ_NORMAL;
	matrix matriz_mundo : packoffset(c205) : OBJETO_MATRIZ_MUNDO;
	Material material[16] : packoffset(c209) : OBJETO_MATERIALES;
	matrix matriz_textura : packoffset(c277) : OBJETO_MATRIZ_TEXTURA;
};
...
struct input_objeto_texturado_esqueletico
{
	float3 POSITION : POSITION0;
	float3 NORMAL : NORMAL0;
	float2 TEXCOORD : TEXCOORD0;
	float3 BLENDWEIGHT : BLENDWEIGHT0;
	float4 BLENDINDICES : BLENDINDICES0;
};

// ========== SHADER ========== //

void vs_objeto_texturado_esqueletico(input_objeto_texturado_esqueletico input, out float4 ModeloPhong : ModeloPhong, out float2 ObjetoTexCoord : ObjetoTexCoord, out float4 ProjectedPosition : SV_POSITION)
{
    float3 TransformedNormal = skinNormal(input.NORMAL, input.BLENDWEIGHT, input.BLENDINDICES);
    float3 ModelViewNormal = modelViewTransformedNormal(TransformedNormal);
    float4 TransformedPosition = skinPosition(input.POSITION, input.BLENDWEIGHT, input.BLENDINDICES);
    float4 ModelViewPosition = modelViewTransformedPosition(TransformedPosition);
